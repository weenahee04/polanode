...
const App: React.FC = () => {
  ...
  return (
    <div className="flex h-[100dvh] w-full flex-col bg-slate-50 relative overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden no-scrollbar pb-24">
        ...
        {activeTab === Tab.HOME && (
          <div className="space-y-6 pb-6 animate-fade-in">
            {/* Header */}
            <div className="sticky top-0 z-20 flex items-center justify-between bg-white/80 px-6 py-4 backdrop-blur-md border-b border-slate-100">
              <div>
                <h1 className="text-xl font-bold text-slate-800 font-kanit">สวัสดี, คุณสมชาย 👋</h1>
                <p className="text-xs text-slate-500 font-kanit">Member Gold Class</p>
              </div>
              <div className="flex gap-3">
                 <button 
                   onClick={() => setShowBrainView(true)}
                   className="relative rounded-full bg-slate-100 p-2 text-slate-600 transition-colors hover:bg-blue-50 hover:text-[#0056b3]"
                 >
            ...
            <div>
              <div className="px-6 pb-2">
                <h2 className="text-sm font-bold text-slate-700 font-kanit">บริการด่วน (Quick Service)</h2>
              </div>
            ...
            <div className="px-5">
               <div className="mb-4 flex items-center justify-between">
                 <h2 className="text-sm font-bold text-slate-700 font-kanit">สิทธิพิเศษแนะนำ (Recommended)</h2>
                 <button 
                    onClick={() => setActiveTab(Tab.REWARDS)}
                    className="text-xs font-medium text-[#0056b3] font-kanit"
                 >
                    ดูทั้งหมด
                 </button>
               </div>
               <div className="grid grid-cols-2 gap-4">
                  {REWARD_ITEMS.slice(0, 2).map(item => (
        ...
        {activeTab === Tab.REWARDS && (
          <div className="min-h-full bg-slate-50 px-5 pt-6 pb-6 animate-fade-in">
             <div className="mb-6">
                <h1 className="text-2xl font-bold text-slate-800 font-kanit">แลกของรางวัล</h1>
                <p className="text-sm text-slate-500 font-kanit">ใช้คะแนนสะสมของคุณแลกรับสิทธิพิเศษ</p>
             </div>
             ...
             <div className="mb-6 flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                {REWARD_CATEGORIES.map(cat => (
                  <button
                    ...
                    className={`whitespace-nowrap rounded-full px-4 py-1.5 text-xs font-medium transition-colors font-kanit ${
        ...
        {activeTab === Tab.AI_CHAT && (
          <div className="flex h-full flex-col bg-slate-50">
            ...
            <div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 shadow-sm z-20">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-tr from-[#0056b3] to-[#00a8e8] shadow-md">
                   <Sparkles className="h-5 w-5 text-white" />
                </div>
                <div>
                  <h2 className="text-sm font-bold text-slate-800 font-kanit">AI Health Assistant</h2>
              ...
              <div className="flex bg-slate-100 rounded-lg p-1 gap-1">
                 <button 
                   ...
                   className={`p-1.5 rounded-md transition-all ${chatViewMode === 'CHAT' ? 'bg-white shadow text-[#0056b3]' : 'text-slate-400'}`}
                 >
                   <MessageCircle className="h-4 w-4" />
                 </button>
                 <button 
                   onClick={() => setChatViewMode('GRAPH')}
                   className={`p-1.5 rounded-md transition-all ${chatViewMode === 'GRAPH' ? 'bg-white shadow text-[#0056b3]' : 'text-slate-400'}`}
                 >
                   <Network className="h-4 w-4" />
                 </button>
                 <button 
                   onClick={() => setChatViewMode('FLOWCHART')}
                   className={`p-1.5 rounded-md transition-all ${chatViewMode === 'FLOWCHART' ? 'bg-white shadow text-[#0056b3]' : 'text-slate-400'}`}
                 >
                   <GitMerge className="h-4 w-4" />
                 </button>
                 <button 
                   onClick={() => setChatViewMode('CHECKLIST')}
                   className={`p-1.5 rounded-md transition-all ${chatViewMode === 'CHECKLIST' ? 'bg-white shadow text-[#0056b3]' : 'text-slate-400'}`}
                 >
            ...
            <div className="flex-1 overflow-hidden relative">
              ...
              <div className={`absolute inset-0 flex flex-col ${chatViewMode === 'CHAT' ? 'z-10 opacity-100' : 'z-0 opacity-0 pointer-events-none'}`}>
                 <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
                    {messages.map((msg, index) => (
                      <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in-up`}>
                        <div className={`max-w-[80%] rounded-2xl px-4 py-3 shadow-sm font-kanit text-sm leading-relaxed ${
                          msg.role === 'user' 
                          ...
                          {msg.imageUrl && (
                            <img src={msg.imageUrl} alt="User Upload" className="mb-2 rounded-lg max-h-48 object-cover w-full border border-white/20" />
                          )}
                          {msg.text}
                        </div>
                      </div>
                    ...
                    {isLoading && (
                       <div className="flex justify-start animate-pulse">
                         <div className="rounded-2xl bg-slate-200 px-4 py-3 rounded-bl-none">
                 ...
                 {/* Input Area */}
                 <div className="bg-white p-3 border-t border-slate-100 flex items-center gap-2 pb-24">
                    <button 
                      onClick={() => fileInputRef.current?.click()}
                      className="rounded-full bg-slate-100 p-2.5 text-slate-500 hover:bg-slate-200 transition-colors"
                    >
                    ...
                    <div className="relative flex-1">
                      <input 
                        ...
                        className="w-full rounded-full border border-slate-200 bg-slate-50 py-2.5 pl-4 pr-10 text-sm font-kanit focus:border-[#0056b3] focus:outline-none"
                      />
                    </div>
                    <button 
                      onClick={handleSendMessage}
                      disabled={!inputText.trim() && !isLoading}
                      className="rounded-full bg-[#0056b3] p-2.5 text-white shadow-md shadow-blue-500/20 disabled:opacity-50 hover:bg-blue-700 transition-colors"
                    >
              ...
              <div className={`absolute inset-0 bg-slate-50 ${chatViewMode === 'GRAPH' ? 'z-10' : 'z-0 hidden'}`}>
                  ...
                  {/* Legend Overlay */}
                  <div className="absolute top-4 left-4 bg-white/90 backdrop-blur p-3 rounded-xl shadow-sm border border-slate-100 text-xs font-kanit space-y-2 pointer-events-none">
                     <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-orange-500"></span> Symptom</div>
                     <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-red-500"></span> Disease</div>
                     <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-green-500"></span> Medicine</div>
                  </div>
              </div>
        ...
        {activeTab === Tab.PROFILE && (
          <div className="p-6 bg-slate-50 animate-fade-in">
             <div className="flex flex-col items-center mb-8">
                <div className="h-24 w-24 rounded-full bg-gradient-to-br from-slate-200 to-slate-300 p-1 mb-4">
                   <div className="h-full w-full rounded-full bg-white flex items-center justify-center overflow-hidden">
                      <User className="h-12 w-12 text-slate-400" />
                   </div>
                </div>
                <h2 className="text-xl font-bold text-slate-800 font-kanit">สมชาย ใจดี</h2>
                <p className="text-sm text-slate-500 font-kanit">somchai.j@example.com</p>
             </div>
             ...
             <div className="space-y-3">
                {[
                  ...
                  <button key={idx} className="w-full flex items-center justify-between p-4 bg-white rounded-xl shadow-sm border border-slate-100 hover:bg-slate-50 transition-colors">
                     <div className="flex items-center gap-3">
                        <div className="p-2 rounded-full bg-slate-100 text-slate-600">
                           <item.icon className="h-5 w-5" />
                        </div>
                        <span className="text-sm font-medium text-slate-700 font-kanit">{item.label}</span>
                     </div>
                ...
                <button 
                  ...
                  className="w-full flex items-center justify-between p-4 mt-6 bg-red-50 rounded-xl border border-red-100 hover:bg-red-100 transition-colors"
                >
                   <div className="flex items-center gap-3">
                      <div className="p-2 rounded-full bg-red-100 text-red-500">
                         <LogOut className="h-5 w-5" />
                      </div>
                      <span className="text-sm font-bold text-red-500 font-kanit">ออกจากระบบ</span>
                   </div>
                </button>
             </div>
          </div>
      ...
      {/* Bottom Navigation */}
      <div className="fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 px-6 py-3 pb-8 z-50 flex justify-between items-center shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
         <button 
           onClick={() => setActiveTab(Tab.HOME)}
           className={`flex flex-col items-center gap-1 transition-colors ${activeTab === Tab.HOME ? 'text-[#0056b3]' : 'text-slate-400'}`}
         >
            <Home className={`h-6 w-6 ${activeTab === Tab.HOME ? 'fill-current' : ''}`} />
            <span className="text-[10px] font-medium font-kanit">หน้าหลัก</span>
         </button>
         <button 
           onClick={() => setActiveTab(Tab.REWARDS)}
           className={`flex flex-col items-center gap-1 transition-colors ${activeTab === Tab.REWARDS ? 'text-[#0056b3]' : 'text-slate-400'}`}
         >
            <Gift className={`h-6 w-6 ${activeTab === Tab.REWARDS ? 'fill-current' : ''}`} />
            <span className="text-[10px] font-medium font-kanit">แลกของ</span>
         </button>
         ...
         <div className="relative -top-6">
            <button 
              ...
              className="h-14 w-14 rounded-full bg-[#222222] flex items-center justify-center shadow-xl shadow-slate-400/50 hover:scale-105 transition-transform ring-4 ring-slate-50"
            >
         ...
         <button 
           onClick={() => setActiveTab(Tab.AI_CHAT)}
           className={`flex flex-col items-center gap-1 transition-colors ${activeTab === Tab.AI_CHAT ? 'text-[#0056b3]' : 'text-slate-400'}`}
         >
            <MessageCircle className={`h-6 w-6 ${activeTab === Tab.AI_CHAT ? 'fill-current' : ''}`} />
            <span className="text-[10px] font-medium font-kanit">AI Chat</span>
         </button>
         <button 
           onClick={() => setActiveTab(Tab.PROFILE)}
           className={`flex flex-col items-center gap-1 transition-colors ${activeTab === Tab.PROFILE ? 'text-[#0056b3]' : 'text-slate-400'}`}
         >
            <User className={`h-6 w-6 ${activeTab === Tab.PROFILE ? 'fill-current' : ''}`} />
            <span className="text-[10px] font-medium font-kanit">โปรไฟล์</span>
         </button>
      </div>
    </div>
  );
